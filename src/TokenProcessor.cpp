...
// Hoisted from inside tokenize() — was re-initialized on every FUNC_DEF token.
// Declared once here; O(1) lookup in the hot path via linear scan over 14 entries.
...
string TokenProcessor::cleanLine(const string& line) const {
    return trim(stripComment(line));
}

void TokenProcessor::emitBraceTokens(const string& clean, int lineNum,
                                      vector<Token>& out) const {
    for (char c : clean) {
        Token t;
        t.lineNumber = lineNum;
        if      (c == '{') { t.type = TokenType::OPEN_BRACE;  t.text = "{"; out.push_back(t); }
        else if (c == '}') { t.type = TokenType::CLOSE_BRACE; t.text = "}"; out.push_back(t); }
        else if (c == '(') { t.type = TokenType::OPEN_PAREN;  t.text = "("; out.push_back(t); }
        else if (c == ')') { t.type = TokenType::CLOSE_PAREN; t.text = ")"; out.push_back(t); }
    }
}

vector<string> TokenProcessor::extractWords(const string& clean) const {
    vector<string> words;
    string word;
...
void TokenProcessor::emitBitToken(const string& clean, int lineNum,
                                   vector<Token>& out) const {
    bool hasBitPattern =
...
vector<Token> TokenProcessor::tokenize(const string& code) {
    ...
    while (getline(stream, line)) {
        ...
        auto bc = line.find("/*");
        if (bc != string::npos && line.find("*/", bc + 2) == string::npos) {
            inBlockComment = true;
            continue;
        }

        const string clean = cleanLine(line);
        if (clean.empty()) continue;

        emitBraceTokens(clean, lineNum, tokens);
        emitBitToken(clean, lineNum, tokens);

        const vector<string> words = extractWords(clean);
        for (size_t i = 0; i < words.size(); ++i) {
            ...
            // Collect user-defined function names for recursion detection
            if (tt == TokenType::FUNC_DEF) {
                bool isKw = false;
                for (const auto& k : kKeywords) {
                    if (w == k) { isKw = true; break; }
                }
                if (!isKw && w.size() > 1) {
